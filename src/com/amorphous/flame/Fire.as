package com.amorphous.flame
{
	...
	public class Fire extends Sprite
	{
		 ...
		 */
		public function Fire(X:uint, Y:uint, gridWidth:uint,gridHeight:uint,numCellsX:uint,numCellsY:uint,particleImage:Bitmap = null)
		{
		...
		public function render(e:Event):void 
		{
			...
			{
				for (var num:Number = 0; num < 2; num += 1)
				{
					fireCreator.addParticles(fireFields, partSize);
		 ...
		 */
		public function burnFuelPoint(x:int,y:int)
		{
			fireFields.fuel.r[fireFields.heat.getIndexForXYPosition(x,y,sw,sh)] = 1;
			fireFields.heat.r[fireFields.heat.getIndexForXYPosition(x,y,sw,sh)] = 1;
		}
		 ...
		 * @param	Y					Y position of the cell
		 */
		public function addFuelPoint(x:int,y:int)
		{
			fireFields.fuel.r[fireFields.heat.getIndexForXYPosition(x,y,sw,sh)] = 1;
		}
		
		/**
		 * burnFuelCircle: Creates a circle of fuel and heat at the given point
		 ...
		 */
		public function burnFuelCircle(x:int, y:int, radius:Number)
		{
			var i:int = x;
			var j:int = y;
			...
			for(var yy=-radius; yy<=radius; yy+=1)
			{
				...
				for(var xx=-radius; xx<=radius; xx+=1)
				{
					...
					if(xx*xx+yy*yy <= radius*radius)
					{
						...
						fireFields.heat.r[fireFields.fuel.getIndexForCellPosition(X+xx,Y+yy)] = 1;
					}
				}
			}
		}
		...
		/**
		 * addFuelCircle: Creates a circle of fuel at the given point
		 * @param	x					X position of the center of the circle. (Nearest cell)
		 * @param	Y					Y position of the center of the circle. (Nearest cell)
		 * @param	radius				Radius of the circle (in number of cells)
		 */
		public function addFuelCircle(x:int, y:int, radius:Number)
		{
			var i:int = x;
			var j:int = y;
			i = Math.floor(i / (sw / fireFields.width));
