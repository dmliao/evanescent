package net.flashpunk
{
	...
	public class Entity extends Tweener
	{
		 ...
		 */
		public function Entity(x:Number = 0, y:Number = 0, graphic:Graphic = null, mask:Mask = null, layer:Number = 0) 
		{
			this.x = x;
			this.y = y;
		...
		public function collideLine(fromX:int, fromY:int, toX:int, toY:int, precision:uint = 1):Boolean
		{
			// If the distance is less than precision, do the short sweep.
			if (precision < 1) precision = 1;
			if (FP.distance(fromX, fromY, toX, toY) < precision)
			{
				return collidePoint(this.x, this.y, fromX, toY);
			}
			...
			if (xDelta > yDelta)
			{
				...
				if (xSign > 0)
				{
					while (x < toX)
					{
						if(collidePoint(this.x, this.y, x, y))
						{
							return true;
						}
						x += xSign; y += ySign;
			...
			if (precision > 1)
			{
				return collidePoint(this.x, this.y, toX, toY);
			}
